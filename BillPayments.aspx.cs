...
public partial class BillPayments : System.Web.UI.Page
{
    ...
    */
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        SessionVariables.CurrentPage = "BillPayments.aspx";
    ...
    public void DueData()
     {
        ...
        DataSet ds = bill.GetLatestBills(muser.currentResident.FlatNumber, "Show All","","");

        if (ds == null)
        {
            lblResBillsEmptyText.Text = "Error Retreiving Data";
        }
        else if (ds.Tables[0].Rows.Count == 0)
        {
            lblResBillsEmptyText.Text = "There are no bills";
        }
        else
        {
    ...
    protected void btnpaymannual_Click(object sender, EventArgs e)
    {
        ...
        DateTime  AmountPaidDate = Utility.GetCurrentDateTimeinUTC();
        int TransactionID = Convert.ToInt32(txtTransaID.Text);
        ...
        String mode = drpPayMode.SelectedItem.Text;

        Bill bill = new Bill();

        bool result = bill.UpdatePayment(PayID,Amount,mode,TransactionID,0);
        if (result == true)
        {
